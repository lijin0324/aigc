...
app = Flask(__name__)
CORS(app)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///draw.db'
...
def get_title_by_level():
    # 从查询字符串中获取 level 参数
    level = request.args.get('level')

    # 根据 level 过滤数据库记录
    random_instance = describe.query.filter_by(level=level).order_by(func.random()).first()

    if random_instance:
        title = random_instance.title
        return jsonify({"title": title})
    else:
        return jsonify({"title": "No record found for level " + str(level)})
...
def save_title():
    ...
    if not (question and level):
        return jsonify({"error": "Missing required fields"}), 400

    # 创建一个新的记录并保存到数据库
    new_record = describe(level=level, title=question)
